...
$.entwine('ss', function($) {
  $('.grid-field').entwine({
    ...
    showDetailView: function(url, event) {
      this.maybeOpenUrlInNewTab(event, url, () => window.location.href = url);
    },
    maybeOpenUrlInNewTab: function(event, url, openInSameTabCallback) {
  ...
  $('.grid-field .action.no-ajax, .grid-field .no-ajax .action:button').entwine({
    onclick: function(event) {
      ...
      this.getGridField().maybeOpenUrlInNewTab(event, url, () => window.location.href = url);
